...
public class DeepHedging {
	...
	private void constructNetwork() {
		 ...
		 */
		.addLayer(stringForFirstHiddenLayer, new DenseLayer.Builder().nIn(1).nOut(numberOfNodesForFirstLayer).activation(new ActivationReLU()).build(), stringForPrice)
		//here the number of inputs must be equal to the number of outputs of the layer above, Otherwise, we get an exception at running time 
		.addLayer(stringForSecondHiddenLayer, new DenseLayer.Builder().nIn(numberOfNodesForFirstLayer).nOut(numberOfNodesForSecondLayer).activation(new ActivationReLU()).build(), stringForFirstHiddenLayer)
		//this layer outputs the first strategy, i.e., the first amount of money we sell or buy for the underlying asset. Note we deal with this in the vertex created by addVertex(stringForStrategyValue..
		...
		/*
		 * Now we proceed witha for loop which runs over time in order to construct the rest of the sub-networks. Each sub-network will provide the optimal strategy
		 * at the specific time. In the meantime, we also have to take care of updating the cumulative value of the strategy, because this will contribute to the
		 * loss function which we need to train the network      
		 */
		for(int timeIndex = 1; timeIndex < numberOfTimes - 1; timeIndex ++) {
			...
			/*
			 * Note how a vertex is defined: string which identifies this vertex, name of the operation that must be performed on the vertex to get
			 * the value of this vertex, and string which identifies the vertex on which the operation must be performed.
			 * So: in this case, based on the price, we compute the increment with respect to the last price.. 
			 */
			...
			builder.addLayer(stringForStrategy,new DenseLayer.Builder().nIn(numberOfNodesForSecondLayer).nOut(1).activation(new ActivationIdentity()).build(), stringForSecondHiddenLayer);			 
	...
	private void trainNetwork() throws CalculationException {
		...
		//in this for loop, we get the prices for every simulation at all times, and we use them to construct the INDArray objects
		for(int timeIndex = 0; timeIndex < numberOfTimes; timeIndex ++) {
			
			//they are kind of dummy matrices of one column: the constructor of NDArray accepts matrices, not arrays
			double[][] pricesAtTimeToTrain = new double[numberOfSimulations][1];

			//we will all rows with the corresponding prices..
	 ...
	 * @param seedForTesting, the seed for teh generator of the paths
